...
% Last Modified by GUIDE v2.5 19-Mar-2019 16:16:37
...
%         return;
        handles.drawings = state.drawings;
        handles.areaOfDrawn = state.areaOfDrawn;
        handles.scanSize = state.scanSize;
        handles.filename_1 = state.filename_1;
        handles.filename_2 = state.filename_2;
        handles.path1 = state.path1;
        handles.path2 = state.path2;
%         handles.numSaved = state.numSaved;
        handles.position = state.position;
        handles.areaOfDrawnRegion2 = state.areaOfDrawnRegion2;
        handles.perm = state.perm;
    end
end
...
if question_mat == 1
    for i = 1:size(handles.drawings,2)
        temp = handles.position{1,i};
        xy = temp{1};
        x = xy(:, 1);
        y = xy(:, 2);
%         axes(handles.BscanPlot)
%         hold on
        ...
        plot([x(1) x(size(x,1))],[y(1) y(size(x,1))], 'c-','LineWidth',1);
%         hold off
    end
end
...
elseif question_mat == 1
    volume = state.volume;
    handles.areaOfDrawn = handles.perm;
    ono = uicontrol('Style','edit','Position',[1060 60 75 110],...
        'min',0,'max',2,'enable','inactive');
    set(ono,'String',num2str(handles.areaOfDrawn));
...
if isfield(handles,'perm')
    countDrawings = size(handles.perm,2) + countDrawings;
end
...
%     handles.areaOfDrawn(countDrawings) = areaPerPixel * numOfPixels;
% else
%     sum = handles.numSaved + countDrawings;
%     handles.areaOfDrawn(sum) = areaPerPixel * numOfPixels;
% end
% uicontrol(handles.areaOfDrawnRegion2);
ono = uicontrol('Style','edit','Position',[1060 60 75 110],...
    'min',0,'max',2,'enable','inactive');
set(ono,'String',num2str(handles.areaOfDrawn));
handles.areaOfDrawnRegion2 = ono;
% set(handles.areaOfDrawnRegion2,'String',num2str(handles.areaOfDrawn));
guidata(hObject,handles);
end
...
% % % load (filename, 'handles.drawings');

state.drawings = handles.drawings;
state.areaOfDrawn = handles.areaOfDrawn;
state.scanSize = handles.scanSize;
state.volume = handles.volume;
state.filename_1 = handles.filename_1;
state.filename_2 = handles.filename_2;
state.path1 = handles.path1;
state.path2 = handles.path2;
% state.numSaved = size(handles.drawings,2);
% state.binaryImage = handles.binaryImage;
state.position = handles.position;
state.areaOfDrawnRegion2 = handles.areaOfDrawnRegion2;
state.perm = state.areaOfDrawn;
...
Segs.image = handles.enface;
Segs.drawings = handles.position;
SaveSeg = strcat(fileName,'_drawings');
save(SaveSeg,'Segs');
% textf = strcat(fileName,'.txt');
% fileID = fopen(textf,'w');
% % temp = handles.areaOfDrawn(1,i);
% fprintf(fileID,'%f ',handles.areaOfDrawn);
% fclose(fileID);
